...
class JokerGame:
    ...
    def choose_trump(self):
        ...
        self.trump_suit = first_player.choose_trump()
        print(f"Trump suit chosen: ** {self.trump_suit} **")
    ...
    def play_rounds(self):
        starting_player_index = (self.current_dealer_index + 1) % 4
        for _ in range(9):
            starting_player_index = self.play_single_round(starting_player_index)

    def play_single_round(self, starting_player_index):
        played_cards = []
        for i in range(4):
            player_index = (starting_player_index + i) % 4
            player = self.players[player_index]
            played_cards.append(player.play_card(played_cards, self.trump_suit))
        winner_index = self.determine_winner(played_cards, starting_player_index)
        print(f"\n{self.players[winner_index].name} wins this round.")
        self.players[winner_index].add_round_win()
        return winner_index

    def determine_winner(self, played_cards, starting_player_index):
        ...
        for i, card in enumerate(played_cards):
            if card.beats(highest_card, self.trump_suit):
                highest_card = card
                highest_player = i
        return (starting_player_index + highest_player) % 4
